...
export default function TaskPage() {
  ...
  return (<>
    ...
    <Form className="flex flex-col justify-start items-start h-full w-full gap-6 p-4 px-8 dark:bg-gray-950 bg-white font-Estedad" onSubmit={handleSubmit}>
      ...
      <div className="flex flex-col-reverse justify-between items-end gap-1 mt-3 border-b border-b-gray-800 w-full pb-6">
      ...
      <div className="flex flex-row-reverse w-full justify-end items-start gap-2">
        {!isCompleted && <button className="text-white cursor-pointer bg-red-500 py-2 px-3 rounded-md hover:bg-red-600 flex flex-row items-center justify-between gap-2" onClick={handleDelete}>
          <span>حذف</span>
        ...
        {!isCompleted && <button className="text-white cursor-pointer bg-indigo-500 py-2 px-3 rounded-md hover:bg-indigo-600 flex flex-row items-center justify-between gap-2" type="submit">
          <span>ویرایش</span>
